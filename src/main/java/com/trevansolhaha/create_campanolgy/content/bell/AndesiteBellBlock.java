...
public class AndesiteBellBlock extends ModBaseBellBlock {
    ...
    public @Nullable BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        return ModBlockEntities.ANDESITE_ALLOY_BELL.get().create(blockPos, blockState);
    }
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hitResult) {
        if (!level.isClientSide()) {
            ...
            if (blockEntity instanceof AndesiteBellBlockEntity andesiteBellBlockEntity) {
                andesiteBellBlockEntity.triggerAnim("click_controller", "trigger_click");
            }
        }
