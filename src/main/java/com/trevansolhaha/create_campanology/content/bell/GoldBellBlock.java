...
public class GoldBellBlock extends ModBaseBellBlock {
    ...
    public GoldBellBlock(Properties properties) { super(properties); }

    @Override
    public @Nullable BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        return ModBlockEntities.GOLD_BELL.get().create(blockPos, blockState);
    }
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hitResult) {
        if (!level.isClientSide()) {
            ...
            if (blockEntity instanceof GoldBellBlockEntity goldBellBlockEntity) {
                goldBellBlockEntity.triggerAnim("click_controller", "trigger_click");
            }
        }
