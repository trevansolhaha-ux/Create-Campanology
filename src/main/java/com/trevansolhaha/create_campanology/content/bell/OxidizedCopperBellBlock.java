...
public class OxidizedCopperBellBlock extends ModBaseBellBlock {
    ...
    public OxidizedCopperBellBlock(Properties properties) { super(properties); }

    @Override
    public @Nullable BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        return ModBlockEntities.OXIDIZED_COPPER_BELL.get().create(blockPos, blockState);
    }
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hitResult) {
        if (!level.isClientSide()) {
            ...
            if (blockEntity instanceof OxidizedCopperBellBlockEntity oxidizedCopperBellBlockEntity) {
                oxidizedCopperBellBlockEntity.triggerAnim("click_controller", "trigger_click");
            }
        }
