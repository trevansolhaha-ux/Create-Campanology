...
public class BronzeBellBlock extends ModBaseBellBlock {
    ...
    public BronzeBellBlock(Properties properties) { super(properties); }

    @Override
    public @Nullable BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        return ModBlockEntities.BRONZE_BELL.get().create(blockPos, blockState);
    }
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hitResult) {
        if (!level.isClientSide()) {
            ...
            if (blockEntity instanceof BronzeBellBlockEntity bronzeBellBlockEntity) {
                bronzeBellBlockEntity.triggerAnim("click_controller", "trigger_click");
            }
        }
