...
public class BrassBellBlock extends ModBaseBellBlock {
    ...
    public BrassBellBlock(Properties properties) { super(properties); }

    @Override
    public @Nullable BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        return ModBlockEntities.BRASS_BELL.get().create(blockPos, blockState);
    }
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hitResult) {
        if (!level.isClientSide()) {
            ...
            if (blockEntity instanceof BrassBellBlockEntity brassBellBlockEntity) {
                brassBellBlockEntity.triggerAnim("click_controller", "trigger_click");
            }
        }
